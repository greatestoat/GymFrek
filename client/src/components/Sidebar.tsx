...
function NavItems({ expanded, onNavigate }: NavItemsProps) {
  return (
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center h-20 px-5 relative overflow-hidden">
          ...
          {/* UPDATED: Sleeker Logo Text Style */}
          <span
            className={`font-semibold text-base tracking-[0.18em] uppercase whitespace-nowrap transition-all duration-300 ease-out ${
              expanded
                ? 'opacity-100 max-w-[180px] ml-3.5 translate-x-0'
                : 'opacity-0 max-w-0 ml-0 -translate-x-3'
            }`}
          >
            GYM
            <span
              className="animate-pulse mx-[1px]"
              style={{ color: 'var(--accent)' }}
            >
              _
            </span>
        ...
        {/* UPDATED: Managed Gym Card */}
        {gym && (
          <div
            className={`transition-all duration-300 overflow-hidden px-3 ${
              expanded
                ? 'opacity-100 max-h-20 mb-4 translate-y-0'
                : 'opacity-0 max-h-0 mb-0 -translate-y-2'
            }`}
          >
            <div className="p-3 rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur-md flex flex-col justify-center">
              ...
              <p
                className="font-semibold text-sm tracking-wide capitalize truncate"
      ...
      {mobileOpen && (
        <div
          className="lg:hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-50 transition-opacity animate-in fade-in duration-200"
          onClick={() => setMobileOpen(false)}
        >
          <div
            className="absolute left-0 top-0 h-full w-72 p-5 flex flex-col shadow-2xl animate-in slide-in-from-left duration-300"
          ...
          >
            ...
            {gym && (
              <div className="mb-5 p-3 rounded-xl border border-white/10 bg-white/[0.03]">
                ...
                <p className="font-semibold text-sm tracking-wide capitalize truncate">
                  {gym.name}
                </p>
              </div>
            )}
